...
<<<<<<< HEAD
      $queryListaMateriales = $this->getObjectManager()->createQuery("SELECT m.ID,m.MATERIAL FROM CsnUser\Entity\Inventario m WHERE m.ACTIVO = 1");
=======
...
>>>>>>> 66cfea633e0b110b65b24c5499444913323db719
      ...
      foreach ($estudio[0][0]['TIPOS'] as $est) {
        ...
        foreach ($materialresult as $mat) {
          $categorias[$contador]['materiales'][] = array('id' => $mat['IDMAT'],'material' => $mat['MATERIAL'],'cantidad' => $mat[0]['CANTIDAD'],'razon' => $mat[0]['RAZON']);
        }
        $contador++;
      }
    ...
    public function imagenesestudioAction() {
      if($this->request->isPost()){
        ...
        $queryEstudio = $this->getObjectManager()->createQuery("SELECT e.INTERPRETACION,e.REVISION,es.ID as ESTADO FROM CsnUser\Entity\Estudios e LEFT JOIN e.ESTADO es WHERE e.ID = $idEstudio");
    ...
    public function removerAction() {
      if($this->request->isPost()){
        ...
        $archivo = $this->request->getPost('imagen');
        $extension = $this->request->getPost('ext');
        $idEstudio = $this->request->getPost('estudio');
        $enlace = $om->getRepository('CsnUser\Entity\Estudioimagenes')->findOneBy(array('IMAGEN' => $archivo.'.'.$extension, 'ESTUDIO' => $idEstudio));
        $om->remove($enlace);
        $om->flush();
        $ruta = dirname(dirname(dirname(dirname(dirname(__DIR__))))).'/public/imagenes/Estudios/'.$idEstudio.'/'.$archivo.'.'.$extension;
        $ruta2 = dirname(dirname(dirname(dirname(dirname(__DIR__))))).'/public/imagenes/Estudios/'.$idEstudio.'/'.$archivo.'_th.'.$extension;
        unlink($ruta);
        unlink($ruta2);
        return new JsonModel();
      }
    }
    public function removerrecetaAction() {
      if($this->request->isPost()){
        $om = $this->getObjectManager();
        $archivo = $this->request->getPost('imagen');
        $extension = $this->request->getPost('ext');
        $idEstudio = $this->request->getPost('estudio');
        $enlace = $om->getRepository('CsnUser\Entity\Estudioreceta')->findOneBy(array('IMAGEN' => $archivo.'.'.$extension, 'ESTUDIO' => $idEstudio));
        $om->remove($enlace);
        $om->flush();
        $ruta = dirname(dirname(dirname(dirname(dirname(__DIR__))))).'/public/imagenes/Estudios/'.$idEstudio.'/Receta/'.$archivo.'.'.$extension;
        $ruta2 = dirname(dirname(dirname(dirname(dirname(__DIR__))))).'/public/imagenes/Estudios/'.$idEstudio.'/Receta/'.$archivo.'_th.'.$extension;
        unlink($ruta);
        unlink($ruta2);
        return new JsonModel();
      }
    ...
    public function listacategoriasAction() {
      $this->layout('layout/vacio');
      $objectManager = $this->getObjectManager();
      $query = $objectManager->createQuery("SELECT t,d.NOMBREDEPENDENCIA FROM CsnUser\Entity\Tiposestudio t LEFT JOIN t.DEPENDENCIA d");
    ...
    public function generarestudioAction() {
      ...
      if($this->request->isPost()){
        ...
        foreach (json_decode($materialUtilizado,true) as $mat) {
          ...
          $EstudioMaterial->setRAZON($mat['razon']);
